...
@section('content')
            ...
            <div class="row">
                    ...
                    <article>
                                            ...
                                            @if(!empty(Auth::user()->id))
                                                @if($job -> user_id==Auth::user()->id)
                                                    <td>
                                                        <form method="POST">
                                                            {!! csrf_field() !!}
                                                            <button type="button" id="assignJob"
                                                                    class="btn btn-danger btn-xs"
                                                                    data-post="{{$job -> id }}"
                                                                    data-user="{{$job -> user_id }}"
                                                                    data-comment="{{$jobReply -> user_id }}">Giao Việc
                                                            </button>
                                                        </form>
                                                    </td>
                                                @endif
                        ...
                        <div class="panel panel-primary">
                            ...
                            <div class="panel-body">
                                <form id="commentForm" name="commentForm" action="{{URL::to('/postComment') }}"
                                      method="post">
                                    {!! csrf_field() !!}
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea id="introduce" name="introduce" cols="100%" rows="10"
                                                  style="width=100%;height=100%"></textarea>
                                    </div>
                                    @if(Auth::check() && $job ->active ===1)
                                        <button type="button" class="btn btn-info" disabled="disabled">Hết hạn báo giá
                                        </button>
                                        &nbsp;
                                    @elseif(Auth::check() && $job ->active ===0)
                                        <button type="submit" class="btn btn-info" id="btnInsertComment">Gửi báo giá
                                        </button>
                                    @else
                                        <div class="alert alert-danger" role="alert">Vui lòng đăng nhập để gửi báo giá
                                        </div>
                                    @endif
                ...
                <div class="col-sm-4" id="sidebar">
                    ...
                    <div class="sidebar-widget" id="company">
                        ...
                        <p>
                            <img src="/{{$job->user->avatar}}" style="width:100px"/><br>Người
                            đăng: {{$job -> user -> full_name}}<br>
                            @if(!empty(Auth::user()->id))
    ...
    @if(!empty(Auth::user()->id))
        <div class="popup-box chat-popup" id="message_popup" data-userpost="{{$job -> user -> username}}"
             data-auth="{{Auth::user()->username}}">
            <div class="popup-head">
                <div class="popup-head-left pull-left"><img
                            src="/{{$job -> user -> avatar}}">{{$job -> user -> full_name}} <span id="online"></span>
                </div>
